...
def markPlayer(challengeId, playerId):
    ...
    r = requests.post(url = MARK_PLAYER_API_ENDPOINT, data = data,verify=True)
...
def findUnrankedPlayer(challengeId):
    ...
    # sending post request and saving response as response object
    r = requests.get(url = UNRATED_API_ENDPOINT, data = data,verify=True)
    if r.status_code != 200:
...
def playGame(findData, playerId, byteCode):
    r = requests.get(url = FIND_API_ENDPOINT, data = findData,verify=True)
    if r.status_code != 200:
    ...
    r = requests.post(url = SAVEGAME_API_ENDPOINT, data = gameOutputData,verify=True)
    if r.status_code != 200:
